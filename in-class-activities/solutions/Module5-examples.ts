...
// Insert a student, insert each of their grades (in order), then insert the next student, then their grades, etc.
// until all students are inserted, then fetch transcripts
// export async function importGrades(gradesToImport: ImportTranscript[]): Promise<Transcript[]> {
//   const ret: Transcript[] = [];
// for (const transcript of gradesToImport) {
//   const { studentID } = await client.addStudent(transcript.studentName);
//   for (const grade of transcript.grades) {
...
// Insert a student, then insert each of their grades (in order), then fetch their transcript.
// Do this set of operations asynchronously (concurrently) for all students
// export async function importGrades(gradesToImport: ImportTranscript[]): Promise<Transcript[]> {
//   const promises = gradesToImport.map(async transcript => {
//     const { studentID } = await client.addStudent(transcript.studentName);
//     for (const grade of transcript.grades) {
//       await client.addGrade(studentID, grade.course, grade.grade);
//     }
//     return client.getTranscript(studentID);
//   });
...
// Insert a student, then insert each of their grades asynchronously (concurrently).
// After all students have all of their grades submitted, fetch all of the transcripts asynchronously (concurrently)
export async function importGrades(gradesToImport: ImportTranscript[]): Promise<Transcript[]> {
  const studentIDs = [];
  const insertPromises = gradesToImport.map(async transcript => {
    const { studentID } = await client.addStudent(transcript.studentName);
  ...
  const fetchPromises = studentIDs.map((id) => {
